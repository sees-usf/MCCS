...
public class LPN {
    ...
    /**
     * Return the type (internal, global, ...) of the variable 'var'. 
     * @param var
     * @return
     */
    ...
    //* Return all variables read by this LPN.
    public HashSet<String> getSupport() {
    	HashSet<String> support = new HashSet<String>();
    	for (LPNTran tran : this.transitions) {
    		HashSet<String> tranSupp = tran.getSupportVar();
    		for (String var : tranSupp)
    			support.add(var);
    	}
    	return support;
    }
	...
	public String header() {
		...
		for(String i : this.internals)
			info += "(" + i + " " +  this.varNodeMap.get(i).getIndex(null) + " " + this.getVarNode(i).getType() + "), ";	
		info += "\n";
		...
		info += "\t Variable index map: \t";
		for(int i = 0; i < this.varIndexMap.size(); i++){
			String var = this.varIndexMap.getKey(i);
			info += "(" + var + " " + this.varIndexMap.getValue(var) +"),  ";
		}
		info += "\n";
